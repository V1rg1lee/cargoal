...
impl TemplateRenderer {
    ...
    pub fn render(
        ...
        let template_content = self.find_template(template_name).ok_or_else(|| {
            format!(
                "Template '{}' not found in any configured directories!",
                template_name
            )
        })?;
    ...
    fn find_template(&self, template_name: &str) -> Option<String> {
        println!(
            "Searching for template '{}' in '{:?}'",
            template_name, self.template_dirs
        );
