...
fn test_redirect_slash_to_non_slash() {
    ...
    let client = Client::new();
    let response = client.get("http://localhost:8089/about/").send().unwrap();
    assert_eq!(response.status(), StatusCode::OK);
...
fn test_method_not_allowed() {
    ...
    let client = Client::new();
    let response = client.delete("http://localhost:8093/about").send().unwrap();
...
fn test_custom_headers() {
    ...
    let client = Client::new();
    let response = client.get("http://localhost:8094/about").send().unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    assert_eq!(
        response
            .headers()
